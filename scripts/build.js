...
var lineRegex = /(analytics.load\(|<%= settings\.page %>|}}\(\);)/g;

var snippet = template(source.replace(pageRegex, '$1'), { variable: 'settings' });
var snippetMin = template(minify(source, {
  mangle: { except: ['analytics'] },
  compress: { sequences: false, side_effects: false },
  fromString: true
}).code.replace(pageRegex, '$1').replace(lineRegex, '\n$1'), { variable: 'settings' });
